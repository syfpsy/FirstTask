...
const AddTaskModal = ({ isOpen, onClose }: AddTaskModalProps) => {
  ...
  const [formData, setFormData] = useState({
    ...
    tagsInput: '', // New: Comma-separated tags input
  ...
  const handleSubmit = async (e: React.FormEvent) => { // Made async
    ...
    await addTask({ // Await the async addTask
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg p-6 w-full max-w-md">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <p className="text-sm text-gray-600 dark:text-gray-300">
            Task will be assigned to: <span className="font-medium">{currentUser?.name || currentUser?.email}</span>
          </p>
